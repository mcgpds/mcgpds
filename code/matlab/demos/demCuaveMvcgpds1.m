% DEMCUAVEMVCGPDS1 Run the multi-view CGPDS on the CUAVE dataset.
...
if ~exist('dataType'), dataType = 'CUAVE'; end
...
if ~exist('useScaleVal','var'), useScaleVal = false; end

load('AudioVideoAlignedData1.mat');

dataSetNames={'video','mfcc'};
numberOfDatasets = 2;
Ytr = cell(numberOfDatasets,1);
Ytr{1} = videoTr;
Ytr{2} = mfccTr;
Yts = cell(numberOfDatasets,1);
Yts{1} = videoTs;
Yts{2} = mfccTs;
...
t = linspace(0, 2*pi, length(Ytr{1})+1)'; t = t(1:end-1, 1);
...
timeStampsTraining = [];
dt=0.05;
if ~exist('seqTr','var'), seqTr = []; end
for i=1:length(seqTr)
    t = ([0:(seqTr(i)-prevSeq)].*dt)';
    prevSeq = seqTr(i)+1;
...
dt=t(2)-t(1);
timeStampsTest = ([0:size(Yts{1},1)-1].*dt)';
...
fileToSave = [pwd filesep 'matlab' filesep 'mvcgpdsResultMat' filesep 'dem' dataType 'Mvcgpds' num2str(experimentNo) '.mat'];
...
if isTraining
    ...
    model.fixAlpha = 1;
    model.fixBeta = 1;
    model.fixDynKern = 1; % kernel parameters of private \theta_t
    model.fixShareDynKern = 1; %kernel parameters of shared \theta_t
    iter = 50;
    for i = 1:1
...
if isOptimizingInPrediction == 1
    ...
    [x, varx] = mvcgpdsOptimiseSeqDyn(model.comp{obsMod}, model, vardistx, Y_test, 1,10, samd);
    % keep the optimized variational parameters mu1_star and lambda1_star
    mu1_star = x;
    lambda1_star = varx;
    
    %update shared variational parameters Sq12_star and mu12_star
...
ZpredAll1 = zeros(size(Z_test));
ZpredCovars1 = zeros(size(Z_test));
ZpredCovars2 = zeros(size(Z_test));
ZpredAll2 = zeros(size(Z_test));
indsAll = zeros(size(Z_test,1),1);
...
for i=1:size(Z_test,1)
    ...
    for k=1:numberOfNN
        ...
        [ZpredMu(k,:) Covars(k,:)] = vargplvmPosteriorMeanVar(model.comp{infMod}, x_cur);%, varx_star(i,:)); % varx_star needed?
    end
    ZpredAll1(i,:) = ZpredMu(1,:);
    ZpredCovars1(i,:) = Covars(1,:);
end
fprintf('\n\n');

ZpredAll1 = ZpredAll1.*repmat(model.comp{2}.scale, size(ZpredAll1,1), 1);
ZpredAll1 = ZpredAll1 + repmat(model.comp{2}.bias, size(ZpredAll1,1), 1);
ZpredCovars1 = ZpredCovars1.*repmat(model.comp{2}.scale.*model.comp{2}.scale, size(ZpredCovars1,1), 1);


NNYspace = Ytr{2}(mini,:);
NNXspace = Ytr{2}(indsAllOrig,:);
Errors.NNYspace = calculateRMSE(Ytr{2}(mini,:), Z_test);
Errors.NNXspace = calculateRMSE(Ytr{2}(indsAllOrig,:),Z_test);
Errors.mvcgpds = calculateRMSE(ZpredAll1, Z_test);
%negative likelihood
msll.mvcgpds = calculateMSLL(Z_test,ZpredAll1,ZpredCovars1);
...
save(fileToSave,'model','Y_test','ZpredAll1','ZpredCovars1','Z_test','Errors','msll','mu1_star','lambda1_star','mu12_star');