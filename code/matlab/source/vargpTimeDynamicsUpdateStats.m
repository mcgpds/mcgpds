...
model.dynamics.At = (model.alpha)^2*model.dynamics.Kt + (1-model.alpha)^2*model.epsilon*eye(model.dynamics.N);
...
for q=1:dynModel.q
    ...
    for i=1:length(dynModel.seq)
        seqEnd = seq(i);
        Bt_q = eye(seqEnd-seqStart+1) + LambdaH_q(seqStart:seqEnd,1)*LambdaH_q(seqStart:seqEnd,1)'.*dynModel.At(seqStart:seqEnd,seqStart:seqEnd);
        Lbt_q = jitChol(Bt_q)';
        G = repmat(LambdaH_q(seqStart:seqEnd,1), 1, seqEnd-seqStart+1).*dynModel.At(seqStart:seqEnd,seqStart:seqEnd);
        G = Lbt_q \ G;
        SqOrig(seqStart:seqEnd,q) = diag(dynModel.At(seqStart:seqEnd,seqStart:seqEnd)) - sum(G.*G,1)';
        muqOrig(seqStart:seqEnd,q) = dynModel.vardist.means(seqStart:seqEnd,q);
...
muqOrig = dynModel.vardist.means;

SqOrig = zeros(dynModel.N, dynModel.q); % memory preallocation

for q=1:dynModel.q
    LambdaH_q = dynModel.vardist.covars(:,q).^0.5;
    Bt_q = eye(dynModel.N) + LambdaH_q*LambdaH_q'.*dynModel.At;
    
    Lbt_q = jitChol(Bt_q)';

    G = repmat(LambdaH_q, 1, dynModel.N).*dynModel.At;
    G = Lbt_q \ G; 
    % Numerical stable way for diag(inv(inv(dynModel.Kt +
    % diag(dynModel.vardist.covars(:,q))));
    SqOrig(:,q) = diag(dynModel.At) - sum(G.*G,1)';
